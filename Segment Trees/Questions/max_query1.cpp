...
void buildTree(lli s,lli e, lli *arr, lli ind){
    ...
    if(s==e)
    {
        tree[ind].push_back(arr[s]); // segment tree of vectors
        return;
    }
    //recurse
    lli mid = (s+e)/2;
    ...
    // make the node using built-in function
...
lli queryTree(lli qs, lli qe, lli k, lli s, lli e,lli ind){
    ...
    if(s>=qs && e<=qe){
        // query is the superset of the node range
        lli i = lower_bound(all(tree[ind]),k) - tree[ind].begin();
        lli ans = tree[ind].size() - i ; //sorted and hence this will be the numbers >=k
        return ans;
    }
    lli mid = (s+e)/2;
...
int main(){
    ...
    while(q--){
        cin>>l>>r>>k;
        cout<<queryTree(l-1,r-1,k,0,n-1,1)<<endl;
    }
    return 0;
}
