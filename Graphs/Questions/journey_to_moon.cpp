...
lli dfs_size(lli source,map< lli,bool > &visited,unordered_map < lli, list<lli> > &adj){
    lli size = 1;
    visited[source] = true;
    for(auto k:adj[source]){
        if(visited[k] == false)
            size += dfs_size(k,visited,adj);
    }
    return size;
}
lli solve(lli n, unordered_map < lli, list<lli> > &adj){
    map <lli, bool> visited;
    // goal -> find the size of each connected component;
    // ans is ->
    // ((sum(sizes))**2 - sum(sizes**2) )/ 2
    // divided cause of double counts .
    lli sum = 0;
    lli ans = 0;
    for(auto k:adj)
