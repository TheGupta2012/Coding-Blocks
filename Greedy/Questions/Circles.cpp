...
int main(){
    ...
    sort(all(circles),comp);
    // i have circles arranged according
    // to their x co-ordinates along a line
    bool removed[n] = {false};
    // in the start all are present on the
    // line

    // DO i remove a circle or i Don't?
    // let's see..
    // If i have a circle with overlapping
    // circles it not ALWAYS optimal to remove
    // it .
    // Think more!...
    for(int i=0;i<n;i++){
        if(i==0){
            // how to decide whether to keep
            // the first one ?
