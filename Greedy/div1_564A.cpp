...
bool canSolve(lli swaps,lli n, lli *a, lli *b){
    // just try to solve the problem in swaps steps .
    lli curr = 0;

}
lli solve(lli s,lli e,lli n, lli *a,lli *b){
    lli ans,mid;
    while(s<=e){
        mid= (s+e)/2;
        if(canSolve(mid,n,a,b)==true){
            ans = mid;
            e = mid-1;
        }
        else{
...
int main(){
    ...
    for0(n)
        cin>>b[i];
    // i found the upper limit :D
    lli ans = solve(0,2*n,n,a,b);
    cout<<ans;
