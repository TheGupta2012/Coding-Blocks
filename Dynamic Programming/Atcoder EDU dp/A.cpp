...
{   if(n == 2)
    ...
    // init 
    lli one_hop, two_hop;
    dp[n-1] = 0LL; // if we are at the last element, no cost is required...
    dp[n-2] =  abs(arr[n-2] - arr[n-1]);
    //code...
    for(int i=n-3;i>=0;i--){
        one_hop = abs(arr[i] - arr[i+1]) + dp[i+1];
        two_hop = abs(arr[i] - arr[i+2]) + dp[i+2];

        dp[i] = min(one_hop, two_hop);
    }
