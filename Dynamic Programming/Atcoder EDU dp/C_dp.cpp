...
lli solve(lli n, lli day, char last)
{
    ...
    if (day == n)
    {
        if (last == 'a')
        {   
            return dp[n][0] = max(b[n - 1], c[n - 1]);
        }
        if (last == 'b')
        {
            return dp[n][1] = max(a[n - 1], c[n - 1]);
        }
        if (last == 'c')
        {
            return dp[n][2] = max(a[n - 1], b[n - 1]);
        }
    }
    // works... memoize
    // main code
    if (dp[day][last-'a']!= -1)
        return dp[day][last-'a'];
    if (day == 1LL)
    {
    ...
    else
    {
        if (last == 'a')
        {
            return dp[day][last-'a'] = max(b[day - 1] + solve(n, day + 1, 'b'), c[day - 1] + solve(n, day + 1, 'c'));
        }
        else if (last == 'b')
        {
            return dp[day][last-'a'] = max(a[day - 1] + solve(n, day + 1, 'a'), c[day - 1] + solve(n, day + 1, 'c'));
        }
        else
        {
            return dp[day][last-'a'] = max(b[day - 1] + solve(n, day + 1, 'b'), a[day - 1] + solve(n, day + 1, 'a'));
        }
    }
}
int main()
{
    ...
    // dp[i][j] represents the maximum cost that I can achieve 
    // when I am currently at day i and the last day's activity
    // was chosen as j -> 'a','b','c'...
