...
lli dp[1005][1005];
lli solve_min_coin_change(lli n,lli *coins,lli target){
    // init the dp 
    for(int i=0;i<=n;i++){
...
{   ios::sync_with_stdio(false);
    ...
    lli ans = solve_min_coin_change(n,coins,target);
    if(ans!= -1)
        cout<<"Number of coins is :"<<ans;
    else 
        cout<<"Not possible to form change";
    return 0;
}